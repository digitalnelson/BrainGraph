...
{
	GroupGraph::GroupGraph(int nVerts, GraphLookup* lu) : _adjMtx(nVerts)
	{
		_nVerts = nVerts;
		_lu = lu;
	}

	GroupGraph::~GroupGraph(void)
	{}

	void GroupGraph::AddEdge(int i, int j, GroupEdge val)
	{
		if(i >= _nVerts)
	...
	GroupEdge GroupGraph::GetEdge(int i, int j)
	{
		int idx = _lu->GetEdge(i, j);
		return Edges[idx];
	}

	void GroupGraph::ComputeComponents(vector<int> &edgeIdxs)
	{
